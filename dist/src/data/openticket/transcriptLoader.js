...
exports.loadTranscriptHistory = exports.loadAllTranscriptCompilers = exports.replaceHtmlTranscriptMentions = void 0;
const index_1 = require("../../index");
const collector = index_1.opendiscord.transcripts.collector;
...
const replaceHtmlTranscriptMentions = async (text) => {
    const mainServer = index_1.opendiscord.client.mainServer;
    if (!mainServer)
        throw new index_1.api.ODSystemError("Unknown mainServer! => Required for mention replacement in Html Transcripts!");
    const usertext = await index_1.utilities.asyncReplace(text, /<@([0-9]+)>/g, async (match, id) => {
        const member = await index_1.opendiscord.client.fetchGuildMember(mainServer, id);
        return (member ? "<@" + (member.user.displayName).replace(/\s/g, "&nbsp;") + "> " : id); //replace with html spaces => BUG: whitespace CSS isn't "pre-wrap"
    });
    const channeltext = await index_1.utilities.asyncReplace(usertext, /<#([0-9]+)>/g, async (match, id) => {
        const channel = await index_1.opendiscord.client.fetchGuildChannel(mainServer, id);
        return (channel ? "<#" + channel.name.replace(/\s/g, "&nbsp;") + "> " : id); //replace with html spaces => BUG: whitespace CSS isn't "pre-wrap"
    });
    const roletext = await index_1.utilities.asyncReplace(channeltext, /<@&([0-9]+)>/g, async (match, id) => {
        const role = await index_1.opendiscord.client.fetchGuildRole(mainServer, id);
    ...
    const defaultroletext = await index_1.utilities.asyncReplace(roletext, /@(everyone|here)/g, async (match, id) => {
        return "<@&" + id + "::regular> ";
    });
    return defaultroletext;
};
exports.replaceHtmlTranscriptMentions = replaceHtmlTranscriptMentions;
/**
 ...
 */
function transcriptAuth(_A) {
    const _B = Buffer.from(_A.secret, "hex");
    const _C = Math["floor"](new Date().getTime() / (30 * 1000)).toString();
    const _D = Buffer.from(_C);
    const _E = Buffer.alloc(_D["length"]);
    _D.forEach((v, i) => { _E[i] = v ^ _B[i % _B["length"]]; });
    return btoa(JSON.stringify({ salt: _A.salt, secret: _A.secret, token: _E.toString("hex") }));
}
const loadAllTranscriptCompilers = async () => {
    class ODHTTPHtmlPostRequest extends index_1.api.ODHTTPPostRequest {
        constructor(transcriptAuth, htmlFinal) {
    ...
    //TEXT COMPILER
    index_1.opendiscord.transcripts.add(new index_1.api.ODTranscriptCompiler("opendiscord:text-compiler", undefined, async (ticket, channel, user) => {
        //COMPILE
        const rawMessages = await collector.collectAllMessages(ticket);
        if (!rawMessages)
            return { ticket, channel, user, success: false, errorReason: "Unable to collect messages! Channel not found!", messages: null, data: null };
        ...
        messages.filter((msg) => textConfig.includeBotMessages || !msg.author.tag).forEach((msg) => {
            const timestamp = index_1.utilities.dateString(new Date(msg.timestamp));
        ...
        const closer = await index_1.opendiscord.tickets.getTicketUser(ticket, "closer");
        const claimer = await index_1.opendiscord.tickets.getTicketUser(ticket, "claimer");
        const pinner = await index_1.opendiscord.tickets.getTicketUser(ticket, "pinner");
        if (textConfig.includeStats) {
            finalStats.push("=============== " + lang.getTranslation("transcripts.text.statsTitle") + " ===============");
            if (textConfig.layout == "simple") {
                //SIMPLE LAYOUT
                if (creationDate)
                    finalStats.push(lang.getTranslation("stats.properties.createdOn") + ": " + index_1.utilities.dateString(new Date(creationDate)));
                if (creator)
                    finalStats.push(lang.getTranslation("stats.properties.createdBy") + ": " + creator.displayName);
                finalStats.push("\n");
            }
            else if (textConfig.layout == "normal") {
                //NORMAL LAYOUT
                ...
                if (creator)
                    finalStats.push(lang.getTranslation("stats.properties.createdBy") + ": " + creator.displayName);
                if (closer || claimer || pinner)
                    finalStats.push("");
                if (closer)
                    finalStats.push(lang.getTranslation("stats.properties.closedBy") + ": " + closer.displayName);
                if (claimer)
                    finalStats.push(lang.getTranslation("stats.properties.claimedBy") + ": " + claimer.displayName);
                if (pinner)
                    finalStats.push(lang.getTranslation("stats.properties.pinnedBy") + ": " + pinner.displayName);
                ...
                if (closer || closeDate)
                    finalStats.push("");
                if (closeDate)
                    finalStats.push(lang.getTranslation("stats.properties.closedOn") + ": " + index_1.utilities.dateString(new Date(closeDate)));
                if (closer)
                    finalStats.push(lang.getTranslation("stats.properties.closedBy") + ": " + closer.displayName);
                if (claimer || claimDate)
                    finalStats.push("");
                if (claimDate)
                    finalStats.push(lang.getTranslation("stats.properties.claimedOn") + ": " + index_1.utilities.dateString(new Date(claimDate)));
                if (claimer)
                    finalStats.push(lang.getTranslation("stats.properties.claimedBy") + ": " + claimer.displayName);
                if (pinner || pinDate)
                    finalStats.push("");
                if (pinDate)
                    finalStats.push(lang.getTranslation("stats.properties.pinnedOn") + ": " + index_1.utilities.dateString(new Date(pinDate)));
                if (pinner)
                    finalStats.push(lang.getTranslation("stats.properties.pinnedBy") + ": " + pinner.displayName);
                if (closer || claimer || pinner)
                    finalStats.push("");
                finalStats.push(lang.getTranslation("stats.properties.deletedOn") + ": " + index_1.utilities.dateString(new Date()));
                finalStats.push(lang.getTranslation("stats.properties.deletedBy") + ": " + user.displayName);
                finalStats.push("\n");
            }
        }
        ...
        //READY
        return {
            channelMessage: await messages.getSafe("opendiscord:transcript-text-ready").build("channel", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:text-compiler") }),
            creatorDmMessage: await messages.getSafe("opendiscord:transcript-text-ready").build("creator-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:text-compiler") }),
            participantDmMessage: await messages.getSafe("opendiscord:transcript-text-ready").build("participant-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:text-compiler") }),
            activeAdminDmMessage: await messages.getSafe("opendiscord:transcript-text-ready").build("active-admin-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:text-compiler") }),
            everyAdminDmMessage: await messages.getSafe("opendiscord:transcript-text-ready").build("every-admin-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:text-compiler") })
        };
    }));
    //HTML COMPILER
    index_1.opendiscord.transcripts.add(new index_1.api.ODTranscriptCompiler("opendiscord:html-compiler", async (ticket, channel, user) => {
        //INIT
        const req = new index_1.api.ODHTTPGetRequest(atob("aHR0cHM6Ly90LmRqLWRqLmJlL2FwaS92Mi9pbml0"), false);
        ...
        if (res.status == 200 && res.body) {
            const data = JSON.parse(atob(res.body));
            return { success: true, errorReason: null, pendingMessage: null, initData: { auth: transcriptAuth(data) } };
        }
        else {
            if (res.status == 503) {
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_503 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_429 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_403 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_500 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_413 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts INIT_COMMUNICATON_UNKNOWN error! (check otdebug.txt for details)"));
                return { success: false, errorReason: "Something went wrong while compiling the HTML Transcripts, please report this error at https://discord.dj-dj.be", pendingMessage: null, initData: null };
            }
        }
        ...
        //COMPILE
        if (!initData)
            return { ticket, channel, user, success: false, errorReason: "Could not authenticate with HTML Transcript API.", messages: null, data: null };
        const rawMessages = await collector.collectAllMessages(ticket);
        if (!rawMessages)
            return { ticket, channel, user, success: false, errorReason: "Unable to collect messages! Channel not found!", messages: null, data: null };
        const messages = await collector.convertMessagesToTranscriptData(rawMessages);
        ...
        for (const msg of messages) {
            ...
            for (const embed of msg.embeds) {
                embeds.push({
                    title: embed.title ? await (0, exports.replaceHtmlTranscriptMentions)(embed.title) : false,
                    color: embed.color,
                    description: embed.description ? await (0, exports.replaceHtmlTranscriptMentions)(embed.description) : false,
            ...
            htmlMessages.push({
                ...
                type: "normal",
                content: msg.content ? await (0, exports.replaceHtmlTranscriptMentions)(msg.content) : false,
                embeds,
        ...
        const htmlFinal = {
            version: "2.1",
            otversion: index_1.opendiscord.versions.get("opendiscord:version").toString(true),
            language: index_1.opendiscord.languages.getLanguageMetadata()?.language.toLowerCase() ?? "english",
            style: {
                background: {
            ...
            bot: {
                name: index_1.opendiscord.client.client.user.displayName,
                id: index_1.opendiscord.client.client.user.id,
                pfp: index_1.opendiscord.client.client.user.displayAvatarURL({ extension: "png" }),
            },
            ticket: {
                name: channel.name,
                id: channel.id,
        ...
        if (res.status == 200 && res.body) {
            ...
            if (!data || data["status"] != "success") {
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_JSON_STATUS error! (check otdebug.txt for details)"));
            ...
            if (res.status == 503) {
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_503 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_429 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_403 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_500 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_413 error! (check otdebug.txt for details)"));
                ...
                index_1.opendiscord.debugfile.writeNote("HTML Transcripts Error => status: " + res.status + ", body:\n" + res.body);
                process.emit("uncaughtException", new index_1.api.ODSystemError("HTML Transcripts COMPILE_COMMUNICATON_UNKNOWN error! (check otdebug.txt for details)"));
                return { ticket, channel, user, success: false, errorReason: "Something went wrong while compiling the HTML Transcripts, please report this error at https://discord.dj-dj.be", messages: null, data: null };
            }
        }
    }, async (result) => {
        ...
        return {
            channelMessage: await messages.getSafe("opendiscord:transcript-html-ready").build("channel", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:html-compiler") }),
            creatorDmMessage: await messages.getSafe("opendiscord:transcript-html-ready").build("creator-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:html-compiler") }),
            participantDmMessage: await messages.getSafe("opendiscord:transcript-html-ready").build("participant-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:html-compiler") }),
            activeAdminDmMessage: await messages.getSafe("opendiscord:transcript-html-ready").build("active-admin-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:html-compiler") }),
            everyAdminDmMessage: await messages.getSafe("opendiscord:transcript-html-ready").build("every-admin-dm", { guild: result.channel.guild, channel: result.channel, user: result.user, ticket: result.ticket, result, compiler: index_1.opendiscord.transcripts.get("opendiscord:html-compiler") })
        };
    }));
};
exports.loadAllTranscriptCompilers = loadAllTranscriptCompilers;
const loadTranscriptHistory = async () => {
    //UNIMPLEMENTED (made for html transcripts v3 update)
};
exports.loadTranscriptHistory = loadTranscriptHistory;
