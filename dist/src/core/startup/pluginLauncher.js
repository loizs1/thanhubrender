...
const loadAllPlugins = async () => {
    ...
    index_1.opendiscord.log("Loading plugins...", "system");
    let initPluginError = false;
    if (!fs_1.default.existsSync("./plugins")) {
        index_1.opendiscord.log("Couldn't find ./plugins directory, canceling all plugin execution!", "error");
        return;
    }
    ...
    plugins.forEach((p) => {
        ...
        if (!fs_1.default.statSync("./plugins/" + p).isDirectory())
            return index_1.opendiscord.log("Plugin is not a directory, canceling plugin execution...", "plugin", [
                { key: "plugin", value: "./plugins/" + p }
            ]);
        if (!fs_1.default.existsSync("./plugins/" + p + "/plugin.json")) {
            initPluginError = true;
            index_1.opendiscord.log("Plugin doesn't have a plugin.json, canceling plugin execution...", "plugin", [
                { key: "plugin", value: "./plugins/" + p }
            ]);
            return;
        }
        ...
        try {
            ...
            if (index_1.opendiscord.plugins.exists(rawplugindata.id))
            ...
            index_1.opendiscord.plugins.add(plugin);
        }
        catch (e) {
            //when any of the above errors happen, crash the bot when soft mode isn't enabled
            initPluginError = true;
            index_1.opendiscord.log(e.message + ", canceling plugin execution...", "plugin", [
                { key: "path", value: "./plugins/" + p }
            ]);
            index_1.opendiscord.log("You can see more about this error in the ./otdebug.txt file!", "info");
            index_1.opendiscord.debugfile.writeText(e.stack);
            //try to get some crashed plugin data
            try {
                const rawplugindata = JSON.parse(fs_1.default.readFileSync("./plugins/" + p + "/plugin.json").toString());
                index_1.opendiscord.plugins.unknownCrashedPlugins.push({
    ...
    const sortedPlugins = index_1.opendiscord.plugins.getAll().sort((a, b) => {
    ...
    sortedPlugins.filter((plugin) => plugin.enabled).forEach((plugin) => {
        ...
        plugin.pluginsIncompatible(index_1.opendiscord.plugins).forEach((incompatible) => incompatibilities.push({ from, to: incompatible }));
        plugin.pluginsInstalled(index_1.opendiscord.plugins).forEach((missing) => missingPlugins.push({ id: from, missing }));
        ...
        if (plugin.data.supportedVersions && plugin.data.supportedVersions.length > 0) {
            const currentVersion = index_1.opendiscord.versions.get("opendiscord:version");
    ...
    incompatibilities.forEach((match) => {
        ...
        const fromPlugin = index_1.opendiscord.plugins.get(match.from);
        if (fromPlugin && !fromPlugin.crashed) {
            fromPlugin.crashed = true;
            fromPlugin.crashReason = "incompatible.plugin";
        }
        const toPlugin = index_1.opendiscord.plugins.get(match.to);
        if (toPlugin && !toPlugin.crashed) {
            toPlugin.crashed = true;
            toPlugin.crashReason = "incompatible.plugin";
        }
        index_1.opendiscord.log(`Incompatible plugins => "${match.from}" & "${match.to}", canceling plugin execution...`, "plugin", [
    ...
    missingDependencies.forEach((match) => {
        const plugin = index_1.opendiscord.plugins.get(match.id);
        if (plugin && !plugin.crashed) {
            plugin.crashed = true;
            plugin.crashReason = "missing.dependency";
        }
        index_1.opendiscord.log(`Missing npm dependency "${match.missing}", canceling plugin execution...`, "plugin", [
    ...
    missingPlugins.forEach((match) => {
        ...
        index_1.opendiscord.log(`Missing required plugin "${match.missing}", canceling plugin execution...`, "plugin", [
            { key: "path", value: "./plugins/" + match.id }
        ]);
        initPluginError = true;
    });
    ...
    versionIncompatibilities.forEach((match) => {
        ...
        const currentVersion = index_1.opendiscord.versions.get("opendiscord:version").toString();
        index_1.opendiscord.log(`Plugin version incompatibility: plugin requires "${versions}" but current bot version is "${currentVersion}", canceling plugin execution...`, "plugin", [
            { key: "path", value: "./plugins/" + match.id }
        ]);
        initPluginError = true;
    });
    //exit on error (when soft mode disabled)
    if (!index_1.opendiscord.defaults.getDefault("softPluginLoading") && initPluginError) {
        console.log("");
        index_1.opendiscord.log("Please fix all plugin errors above & try again!", "error");
        process.exit(1);
    }
    //preload all events required for every plugin
    for (const plugin of sortedPlugins) {
        if (plugin.enabled)
            plugin.data.events.forEach((event) => index_1.opendiscord.events.add(new index_1.api.ODEvent(event)));
    }
    //execute all working plugins
    for (const plugin of sortedPlugins) {
        const status = await plugin.execute(index_1.opendiscord.debug, false);
        //exit on error (when soft mode disabled)
        if (!status && !index_1.opendiscord.defaults.getDefault("softPluginLoading")) {
            console.log("");
            index_1.opendiscord.log("Please fix all plugin errors above & try again!", "error");
            process.exit(1);
        }
    ...
    for (const plugin of sortedPlugins) {
        ...
        if (plugin.enabled) {
            index_1.opendiscord.debug.debug("Plugin \"" + plugin.id.value + "\" loaded", [
            ...
            index_1.opendiscord.debug.debug("Plugin \"" + plugin.id.value + "\" disabled", [
                { key: "authors", value: authors },
                { key: "version", value: plugin.version.toString() },
                { key: "priority", value: plugin.priority.toString() }
            ]);
