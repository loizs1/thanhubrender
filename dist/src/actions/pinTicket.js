...
const registerActions = async () => {
    index_1.opendiscord.actions.add(new index_1.api.ODAction("opendiscord:pin-ticket"));
    index_1.opendiscord.actions.get("opendiscord:pin-ticket").workers.add([
        new index_1.api.ODWorker("opendiscord:pin-ticket", 2, async (instance, params, source, cancel) => {
            const { guild, channel, user, ticket, reason } = params;
            if (channel.isThread())
                throw new index_1.api.ODSystemError("Unable to pin ticket! Open Ticket doesn't support threads!");
            await index_1.opendiscord.events.get("onTicketPin").emit([ticket, user, channel, reason]);
            //update ticket
            ...
            //update stats
            await index_1.opendiscord.stats.get("opendiscord:global").setStat("opendiscord:tickets-pinned", 1, "increase");
            await index_1.opendiscord.stats.get("opendiscord:user").setStat("opendiscord:tickets-pinned", user.id, 1, "increase");
            ...
            try {
                await index_1.utilities.timedAwait(channel.setName(newName), 2500, (err) => {
                    index_1.opendiscord.log("Failed to rename channel on ticket pin", "error");
                });
            }
            catch (err) {
                await channel.send((await index_1.opendiscord.builders.messages.getSafe("opendiscord:error-channel-rename").build("ticket-pin", { guild, channel, user, originalName, newName })).message);
            }
            ...
            if (ticketMessage) {
                try {
                    ...
                    index_1.opendiscord.log("Unable to edit ticket message on ticket pinning!", "error", [
            ...
            if (params.sendMessage)
                await channel.send((await index_1.opendiscord.builders.messages.getSafe("opendiscord:pin-message").build(source, { guild, channel, user, ticket, reason })).message);
            ticket.get("opendiscord:busy").value = false;
            await index_1.opendiscord.events.get("afterTicketPinned").emit([ticket, user, channel, reason]);
        ...
        new index_1.api.ODWorker("opendiscord:discord-logs", 1, async (instance, params, source, cancel) => {
            ...
            if (generalConfig.data.system.logs.enabled && generalConfig.data.system.messages.pinning.logs) {
                ...
                if (logChannel)
                    logChannel.send(await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-logs").build(source, { guild, channel, user, ticket, mode: "pin", reason, additionalData: null }));
            }
            //to dm
            ...
            if (creator && generalConfig.data.system.messages.pinning.dm)
                await index_1.opendiscord.client.sendUserDm(creator, await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-dm").build(source, { guild, channel, user, ticket, mode: "pin", reason, additionalData: null }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", 0, (instance, params, source, cancel) => {
            const { guild, channel, user, ticket } = params;
            index_1.opendiscord.log(user.displayName + " pinned a ticket!", "info", [
...
const registerVerifyBars = async () => {
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:pin-ticket-ticket-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-ticket-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:pin-ticket-ticket-message").success.add([
        ...
        new index_1.api.ODWorker("opendiscord:pin-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //pin with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:pin-ticket-reason").build("ticket-message", { guild, channel, user, ticket }));
            }
            else {
                //pin without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:pin-ticket").run("ticket-message", { guild, channel, user, ticket, reason: null, sendMessage: true });
    ...
    index_1.opendiscord.verifybars.get("opendiscord:pin-ticket-ticket-message").failure.add([
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:pin-ticket-unpin-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-unpin-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:pin-ticket-unpin-message").success.add([
        new index_1.api.ODWorker("opendiscord:permissions", 1, async (instance, params, source, cancel) => {
            const permissionMode = generalConfig.data.system.permissions.pin;
            if (permissionMode == "none") {
                //no permissions
        ...
        new index_1.api.ODWorker("opendiscord:pin-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //pin with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:pin-ticket-reason").build("unpin-message", { guild, channel, user, ticket }));
            }
            else {
                //pin without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:pin-ticket").run("unpin-message", { guild, channel, user, ticket, reason: null, sendMessage: false });
                await instance.update(await index_1.opendiscord.builders.messages.getSafe("opendiscord:pin-message").build("unpin-message", { guild, channel, user, ticket, reason: null }));
            }
        })
    ]);
    index_1.opendiscord.verifybars.get("opendiscord:pin-ticket-unpin-message").failure.add([
        new index_1.api.ODWorker("opendiscord:back-to-unpin-message", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user } = instance;
    ...
    index_1.opendiscord.actions.get("opendiscord:pin-ticket").workers.backupWorker = new index_1.api.ODWorker("opendiscord:cancel-busy", 0, (instance, params) => {
