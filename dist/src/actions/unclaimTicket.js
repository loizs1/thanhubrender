...
const registerActions = async () => {
    index_1.opendiscord.actions.add(new index_1.api.ODAction("opendiscord:unclaim-ticket"));
    index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").workers.add([
        new index_1.api.ODWorker("opendiscord:unclaim-ticket", 2, async (instance, params, source, cancel) => {
            const { guild, channel, user, ticket, reason } = params;
            if (channel.isThread())
                throw new index_1.api.ODSystemError("Unable to unclaim ticket! Open Ticket doesn't support threads!");
            await index_1.opendiscord.events.get("onTicketUnclaim").emit([ticket, user, channel, reason]);
            //update ticket
                ...
                if (channelCategory !== "") {
                    ...
                    try {
                        ...
                        if (!normalCategory) {
                            ...
                            index_1.opendiscord.log("Ticket Unclaiming Error: Unable to find category! #1", "error", [
                            ...
                            if (normalCategory.children.cache.size >= 49 && channelBackupCategory != "") {
                                ...
                                if (!backupCategory) {
                                    //default category was not found
                                    index_1.opendiscord.log("Ticket Unclaiming Error: Unable to find category! #2", "error", [
                        ...
                        index_1.opendiscord.log("Unable to move ticket to 'unclaimed category'!", "error", [
            ...
            if (ticketMessage) {
                try {
                    ...
                    index_1.opendiscord.log("Unable to edit ticket message on ticket unclaiming!", "error", [
                        { key: "channel", value: "#" + channel.name },
                        { key: "channelid", value: channel.id, hidden: true },
            ...
            if (params.sendMessage)
                await channel.send((await index_1.opendiscord.builders.messages.getSafe("opendiscord:unclaim-message").build(source, { guild, channel, user, ticket, reason })).message);
            ticket.get("opendiscord:busy").value = false;
            await index_1.opendiscord.events.get("afterTicketUnclaimed").emit([ticket, user, channel, reason]);
        ...
        new index_1.api.ODWorker("opendiscord:discord-logs", 1, async (instance, params, source, cancel) => {
            ...
            if (generalConfig.data.system.logs.enabled && generalConfig.data.system.messages.claiming.logs) {
                ...
                if (logChannel)
                    logChannel.send(await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-logs").build(source, { guild, channel, user, ticket, mode: "unclaim", reason, additionalData: null }));
            }
            //to dm
            ...
            if (creator && generalConfig.data.system.messages.claiming.dm)
                await index_1.opendiscord.client.sendUserDm(creator, await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-dm").build(source, { guild, channel, user, ticket, mode: "unclaim", reason, additionalData: null }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", 0, (instance, params, source, cancel) => {
            const { guild, channel, user, ticket } = params;
            index_1.opendiscord.log(user.displayName + " unclaimed a ticket!", "info", [
...
const registerVerifyBars = async () => {
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:unclaim-ticket-ticket-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-ticket-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-ticket-message").success.add([
        ...
        new index_1.api.ODWorker("opendiscord:unclaim-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (!ticket.get("opendiscord:claimed").value) {
                instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:error").build("button", { guild, channel, user, error: index_1.opendiscord.languages.getTranslation("errors.actionInvalid.unclaim"), layout: "simple" }));
                return cancel();
            }
            //return when busy
            ...
            if (params.data == "reason") {
                //unclaim with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:unclaim-ticket-reason").build("ticket-message", { guild, channel, user, ticket }));
            }
            else {
                //unclaim without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").run("ticket-message", { guild, channel, user, ticket, reason: null, sendMessage: true });
    ...
    index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-ticket-message").failure.add([
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:unclaim-ticket-claim-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-claim-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-claim-message").success.add([
        new index_1.api.ODWorker("opendiscord:permissions", 1, async (instance, params, source, cancel) => {
            const permissionMode = generalConfig.data.system.permissions.unclaim;
            if (permissionMode == "none") {
                //no permissions
        ...
        new index_1.api.ODWorker("opendiscord:unclaim-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //unclaim with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:unclaim-ticket-reason").build("claim-message", { guild, channel, user, ticket }));
            }
            else {
                //unclaim without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").run("claim-message", { guild, channel, user, ticket, reason: null, sendMessage: false });
                await instance.update(await index_1.opendiscord.builders.messages.getSafe("opendiscord:unclaim-message").build("claim-message", { guild, channel, user, ticket, reason: null }));
            }
        })
    ]);
    index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-claim-message").failure.add([
        new index_1.api.ODWorker("opendiscord:back-to-claim-message", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user } = instance;
    ...
    index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").workers.backupWorker = new index_1.api.ODWorker("opendiscord:cancel-busy", 0, (instance, params) => {
