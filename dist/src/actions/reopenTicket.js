...
const registerActions = async () => {
    index_1.opendiscord.actions.add(new index_1.api.ODAction("opendiscord:reopen-ticket"));
    index_1.opendiscord.actions.get("opendiscord:reopen-ticket").workers.add([
        new index_1.api.ODWorker("opendiscord:reopen-ticket", 2, async (instance, params, source, cancel) => {
            const { guild, channel, user, ticket, reason } = params;
            if (channel.isThread())
                throw new index_1.api.ODSystemError("Unable to reopen ticket! Open Ticket doesn't support threads!");
            await index_1.opendiscord.events.get("onTicketReopen").emit([ticket, user, channel, reason]);
            //update ticket
            ...
            //update stats
            await index_1.opendiscord.stats.get("opendiscord:global").setStat("opendiscord:tickets-reopened", 1, "increase");
            await index_1.opendiscord.stats.get("opendiscord:user").setStat("opendiscord:tickets-reopened", user.id, 1, "increase");
            //update category
                ...
                if (channelCategory !== "") {
                    ...
                    try {
                        ...
                        if (!normalCategory) {
                            ...
                            index_1.opendiscord.log("Ticket Reopening Error: Unable to find category! #1", "error", [
                            ...
                            if (normalCategory.children.cache.size >= 49 && channelBackupCategory != "") {
                                ...
                                if (!backupCategory) {
                                    //default category was not found
                                    index_1.opendiscord.log("Ticket Reopening Error: Unable to find category! #2", "error", [
                        ...
                        index_1.opendiscord.log("Unable to move ticket to 'reopened category'!", "error", [
            ...
            if (ticketMessage) {
                try {
                    ...
                    index_1.opendiscord.log("Unable to edit ticket message on ticket reopening!", "error", [
                        { key: "channel", value: "#" + channel.name },
                        { key: "channelid", value: channel.id, hidden: true },
            ...
            if (params.sendMessage)
                await channel.send((await index_1.opendiscord.builders.messages.getSafe("opendiscord:reopen-message").build(source, { guild, channel, user, ticket, reason })).message);
            ticket.get("opendiscord:busy").value = false;
            await index_1.opendiscord.events.get("afterTicketReopened").emit([ticket, user, channel, reason]);
        ...
        new index_1.api.ODWorker("opendiscord:discord-logs", 1, async (instance, params, source, cancel) => {
            ...
            if (generalConfig.data.system.logs.enabled && generalConfig.data.system.messages.reopening.logs) {
                ...
                if (logChannel)
                    logChannel.send(await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-logs").build(source, { guild, channel, user, ticket, mode: "reopen", reason, additionalData: null }));
            }
            //to dm
            const creator = await index_1.opendiscord.tickets.getTicketUser(ticket, "creator");
            if (creator && generalConfig.data.system.messages.reopening.dm)
                await index_1.opendiscord.client.sendUserDm(creator, await index_1.opendiscord.builders.messages.getSafe("opendiscord:ticket-action-dm").build(source, { guild, channel, user, ticket, mode: "reopen", reason, additionalData: null }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", 0, (instance, params, source, cancel) => {
            const { guild, channel, user, ticket } = params;
            index_1.opendiscord.log(user.displayName + " reopened a ticket!", "info", [
...
const registerVerifyBars = async () => {
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:reopen-ticket-ticket-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-ticket-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-ticket-message").success.add([
        ...
        new index_1.api.ODWorker("opendiscord:reopen-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //reopen with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:reopen-ticket-reason").build("ticket-message", { guild, channel, user, ticket }));
            }
            else {
                //reopen without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:reopen-ticket").run("ticket-message", { guild, channel, user, ticket, reason: null, sendMessage: true });
    ...
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-ticket-message").failure.add([
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:reopen-ticket-close-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-close-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-close-message").success.add([
        new index_1.api.ODWorker("opendiscord:permissions", 1, async (instance, params, source, cancel) => {
            const permissionMode = generalConfig.data.system.permissions.reopen;
            if (permissionMode == "none") {
                //no permissions
        ...
        new index_1.api.ODWorker("opendiscord:reopen-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //reopen with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:reopen-ticket-reason").build("close-message", { guild, channel, user, ticket }));
            }
            else {
                //reopen without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:reopen-ticket").run("close-message", { guild, channel, user, ticket, reason: null, sendMessage: false });
                await instance.update(await index_1.opendiscord.builders.messages.getSafe("opendiscord:reopen-message").build("close-message", { guild, channel, user, ticket, reason: null }));
            }
        })
    ]);
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-close-message").failure.add([
    ...
    index_1.opendiscord.verifybars.add(new index_1.api.ODVerifyBar("opendiscord:reopen-ticket-autoclose-message", index_1.opendiscord.builders.messages.getSafe("opendiscord:verifybar-autoclose-message"), !generalConfig.data.system.disableVerifyBars));
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-autoclose-message").success.add([
        new index_1.api.ODWorker("opendiscord:permissions", 1, async (instance, params, source, cancel) => {
            const permissionMode = generalConfig.data.system.permissions.reopen;
            if (permissionMode == "none") {
                //no permissions
        ...
        new index_1.api.ODWorker("opendiscord:reopen-ticket", 0, async (instance, params, source, cancel) => {
            ...
            if (params.data == "reason") {
                //reopen with reason
                instance.modal(await index_1.opendiscord.builders.modals.getSafe("opendiscord:reopen-ticket-reason").build("autoclose-message", { guild, channel, user, ticket }));
            }
            else {
                //reopen without reason
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:reopen-ticket").run("autoclose-message", { guild, channel, user, ticket, reason: null, sendMessage: false });
                await instance.update(await index_1.opendiscord.builders.messages.getSafe("opendiscord:reopen-message").build("autoclose-message", { guild, channel, user, ticket, reason: null }));
            }
        })
    ]);
    index_1.opendiscord.verifybars.get("opendiscord:reopen-ticket-autoclose-message").failure.add([
    ...
    index_1.opendiscord.actions.get("opendiscord:reopen-ticket").workers.backupWorker = new index_1.api.ODWorker("opendiscord:cancel-busy", 0, (instance, params) => {
