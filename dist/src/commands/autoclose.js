...
const registerCommandResponders = async () => {
    ...
    index_1.opendiscord.responders.commands.add(new index_1.api.ODCommandResponder("opendiscord:autoclose", generalConfig.data.prefix, /^autoclose/));
    index_1.opendiscord.responders.commands.get("opendiscord:autoclose").workers.add([
        new index_1.api.ODWorker("opendiscord:autoclose", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user, member } = instance;
            //check permissions
            const permsResult = await index_1.opendiscord.permissions.checkCommandPerms(generalConfig.data.system.permissions.autoclose, "support", user, member, channel, guild);
            ...
            if (scope == "disable") {
                ...
                await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:autoclose-disable").build(source, { guild, channel, user, ticket, reason }));
            }
                ...
                await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:autoclose-enable").build(source, { guild, channel, user, ticket, reason, time }));
            }
            ...
            if (ticketMessage) {
                try {
                    ...
                    index_1.opendiscord.log("Unable to edit ticket message on autoclose " + scope + "!", "error", [
        ...
        new index_1.api.ODWorker("opendiscord:logs", -1, (instance, params, source, cancel) => {
            const scope = instance.options.getSubCommand();
            const reason = instance.options.getString("reason", false);
            index_1.opendiscord.log(instance.user.displayName + " used the 'autoclose " + scope + "' command!", "info", [
