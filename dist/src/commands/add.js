...
const registerCommandResponders = async () => {
    ...
    index_1.opendiscord.responders.commands.add(new index_1.api.ODCommandResponder("opendiscord:add", generalConfig.data.prefix, "add"));
    index_1.opendiscord.responders.commands.get("opendiscord:add").workers.add([
        new index_1.api.ODWorker("opendiscord:add", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user, member } = instance;
            //check permissions
            const permsResult = await index_1.opendiscord.permissions.checkCommandPerms(generalConfig.data.system.permissions.add, "support", user, member, channel, guild);
            ...
            if (!participants || participants.find((p) => p.user.id == data.id)) {
                instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:error").build("button", { guild, channel, user, error: index_1.opendiscord.languages.getTranslation("errors.actionInvalid.add"), layout: "simple" }));
                return cancel();
            }
            //start adding user to ticket
            await instance.defer(false);
            await index_1.opendiscord.actions.get("opendiscord:add-ticket-user").run(source, { guild, channel, user, ticket, reason, sendMessage: false, data });
            await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:add-message").build(source, { guild, channel, user, ticket, reason, data }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", -1, (instance, params, source, cancel) => {
            index_1.opendiscord.log(instance.user.displayName + " used the 'add' command!", "info", [
