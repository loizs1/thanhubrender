...
const registerCommandResponders = async () => {
    ...
    index_1.opendiscord.responders.commands.add(new index_1.api.ODCommandResponder("opendiscord:unclaim", generalConfig.data.prefix, "unclaim"));
    index_1.opendiscord.responders.commands.get("opendiscord:unclaim").workers.add([
        new index_1.api.ODWorker("opendiscord:unclaim", 0, async (instance, params, source, cancel) => {
            const { user, member, channel, guild } = instance;
            //check permissions
            const permsResult = await index_1.opendiscord.permissions.checkCommandPerms(generalConfig.data.system.permissions.unclaim, "support", user, member, channel, guild);
            ...
            await index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").run(source, { guild, channel, user, ticket, reason, sendMessage: false });
            await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:unclaim-message").build(source, { guild, channel, user, ticket, reason }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", -1, (instance, params, source, cancel) => {
            index_1.opendiscord.log(instance.user.displayName + " used the 'unclaim' command!", "info", [
...
const registerButtonResponders = async () => {
    ...
    index_1.opendiscord.responders.buttons.add(new index_1.api.ODButtonResponder("opendiscord:unclaim-ticket", /^od:unclaim-ticket/));
    index_1.opendiscord.responders.buttons.get("opendiscord:unclaim-ticket").workers.add(new index_1.api.ODWorker("opendiscord:unclaim-ticket", 0, async (instance, params, source, cancel) => {
        const originalSource = instance.interaction.customId.split("_")[1];
        if (originalSource == "ticket-message")
            await index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-ticket-message").activate(instance);
        else if (originalSource == "claim-message")
            await index_1.opendiscord.verifybars.get("opendiscord:unclaim-ticket-claim-message").activate(instance);
        else
...
const registerModalResponders = async () => {
    ...
    index_1.opendiscord.responders.modals.add(new index_1.api.ODModalResponder("opendiscord:unclaim-ticket-reason", /^od:unclaim-ticket-reason_/));
    index_1.opendiscord.responders.modals.get("opendiscord:unclaim-ticket-reason").workers.add([
        new index_1.api.ODWorker("opendiscord:unclaim-ticket-reason", 0, async (instance, params, source, cancel) => {
            ...
            if (originalSource == "ticket-message") {
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").run(originalSource, { guild, channel, user, ticket, reason, sendMessage: true });
                ...
                await instance.defer("update", false);
                await index_1.opendiscord.actions.get("opendiscord:unclaim-ticket").run(originalSource, { guild, channel, user, ticket, reason, sendMessage: false });
