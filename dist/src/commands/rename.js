...
const registerCommandResponders = async () => {
    ...
    index_1.opendiscord.responders.commands.add(new index_1.api.ODCommandResponder("opendiscord:rename", generalConfig.data.prefix, "rename"));
    index_1.opendiscord.responders.commands.get("opendiscord:rename").workers.add([
        new index_1.api.ODWorker("opendiscord:rename", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user, member } = instance;
            //check permissions
            const permsResult = await index_1.opendiscord.permissions.checkCommandPerms(generalConfig.data.system.permissions.rename, "support", user, member, channel, guild);
            ...
            await index_1.opendiscord.actions.get("opendiscord:rename-ticket").run(source, { guild, channel, user, ticket, reason, sendMessage: false, data: name });
            await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:rename-message").build(source, { guild, channel, user, ticket, reason, data: name }));
        }),
        new index_1.api.ODWorker("opendiscord:logs", -1, (instance, params, source, cancel) => {
            index_1.opendiscord.log(instance.user.displayName + " used the 'rename' command!", "info", [
