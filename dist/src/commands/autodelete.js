...
const registerCommandResponders = async () => {
    ...
    index_1.opendiscord.responders.commands.add(new index_1.api.ODCommandResponder("opendiscord:autodelete", generalConfig.data.prefix, /^autodelete/));
    index_1.opendiscord.responders.commands.get("opendiscord:autodelete").workers.add([
        new index_1.api.ODWorker("opendiscord:autodelete", 0, async (instance, params, source, cancel) => {
            const { guild, channel, user, member } = instance;
            //check permissions
            const permsResult = await index_1.opendiscord.permissions.checkCommandPerms(generalConfig.data.system.permissions.autodelete, "support", user, member, channel, guild);
            ...
            if (!scope || (scope != "disable" && scope != "enable"))
                return;
            if (scope == "disable") {
                const reason = instance.options.getString("reason", false);
                ticket.get("opendiscord:autodelete-enabled").value = false;
                ticket.get("opendiscord:autodelete-days").value = 0;
                await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:autodelete-disable").build(source, { guild, channel, user, ticket, reason }));
            }
                ...
                await instance.reply(await index_1.opendiscord.builders.messages.getSafe("opendiscord:autodelete-enable").build(source, { guild, channel, user, ticket, reason, time }));
            }
            ...
            if (ticketMessage) {
                try {
                    ...
                    index_1.opendiscord.log("Unable to edit ticket message on autodelete " + scope + "!", "error", [
        ...
        new index_1.api.ODWorker("opendiscord:logs", -1, (instance, params, source, cancel) => {
            const scope = instance.options.getSubCommand();
            const reason = instance.options.getString("reason", false);
            index_1.opendiscord.log(instance.user.displayName + " used the 'autodelete " + scope + "' command!", "info", [
