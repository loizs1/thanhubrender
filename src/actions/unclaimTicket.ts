...
export const registerActions = async () => {
    ...
    opendiscord.actions.get("opendiscord:unclaim-ticket").workers.add([
        new api.ODWorker("opendiscord:unclaim-ticket",2,async (instance,params,source,cancel) => {
            ...
            //update stats - decrease claimed count
            await opendiscord.stats.get("opendiscord:global").setStat("opendiscord:tickets-claimed",1,"decrease")
            await opendiscord.stats.get("opendiscord:user").setStat("opendiscord:tickets-claimed",user.id,1,"decrease")

            //update leaderboard - decrease claim count
            ...
            if (leaderboardDb){
                ...
                const newCount = (typeof currentCount === "number" ? currentCount : 1) - 1
                if (newCount > 0){
                    await leaderboardDb.set("claims",user.id,newCount)
                }else{
                    await leaderboardDb.delete("claims",user.id)
                }
            }
    ...
    opendiscord.verifybars.get("opendiscord:unclaim-ticket-ticket-message").success.add([
        new api.ODWorker("opendiscord:permissions",1,async (instance,params,source,cancel) => {
            ...
            if (permissionMode == "none"){
                ...
                const roleIds = permissionMode.split(",").map((r: string) => r.trim()).filter((r: string) => r.length > 0)
                let hasRole = false
                for (const roleId of roleIds){
                    const role = await opendiscord.client.fetchGuildRole(instance.guild,roleId)
        ...
        new api.ODWorker("opendiscord:unclaim-ticket",0,async (instance,params,source,cancel) => {
            ...
            if (claimedBy !== user.id){
                instance.reply(await opendiscord.builders.messages.getSafe("opendiscord:error").build("button",{guild,channel,user,error:"Only the staff member who claimed this ticket can unclaim it.",layout:"simple"}))
                return cancel()
            }
