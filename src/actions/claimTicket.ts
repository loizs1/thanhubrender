...
const checkMultiRolePermission = async (guild: discord.Guild, member: discord.GuildMember, permissionMode: string): Promise<boolean> => {
    ...
    for (const roleId of roleIds){
        ...
        if (role && role.members.has(member.id)) return true
    }
    return false
}
