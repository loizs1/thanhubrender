...
export const registerActions = async () => {
    ...
    opendiscord.actions.get("opendiscord:create-ticket-permissions").workers.add([
        new api.ODWorker("opendiscord:check-roles",5,async (instance,params,source,cancel) => {
            if (!params.option.exists("opendiscord:roles")) return
            const rolesData = params.option.get("opendiscord:roles")
            if (!rolesData) return
            const optionRoles = rolesData.value as string[]
            if (optionRoles.length === 0) return
            ...
            if (hideRoles.length > 0) {
                ...
                if (hasHiddenRole) {
                    ...
                    opendiscord.log(params.user.displayName+" tried to create a ticket but doesn't have the required roles!","info",[
