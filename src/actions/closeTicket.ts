...
export const registerActions = async () => {
    ...
    opendiscord.actions.get("opendiscord:close-ticket").workers.add([
        new api.ODWorker("opendiscord:close-ticket",2,async (instance,params,source,cancel) => {
            ...
            if (!ticket.exists("opendiscord:closed-reason")) ticket.add(new api.ODTicketData("opendiscord:closed-reason", reason ?? null))
            else ticket.get("opendiscord:closed-reason").value = reason ?? null
            ...
            //store original channel name in ticket for DM before renaming
            if (!ticket.exists("opendiscord:original-channel-name")) ticket.add(new api.ODTicketData("opendiscord:original-channel-name", channel.name))
            else {
                const originalNameData = ticket.get("opendiscord:original-channel-name")
        ...
        new api.ODWorker("opendiscord:discord-logs",1,async (instance,params,source,cancel) => {
            ...
            const originalNameData = ticket.get("opendiscord:original-channel-name")
            const originalChannelName = originalNameData ? (originalNameData.value as string) : channel.name
            if (creator && generalConfig.data.system.messages.closing.dm) await opendiscord.client.sendUserDm(creator,await opendiscord.builders.messages.getSafe("opendiscord:ticket-action-dm").build(source,{guild,channel,user,ticket,mode:"close",reason,additionalData:originalChannelName}))
        }),
...
export const registerVerifyBars = async () => {
    ...
    opendiscord.verifybars.get("opendiscord:close-ticket-ticket-message").success.add([
        new api.ODWorker("opendiscord:close-ticket",0,async (instance,params,source,cancel) => {
            ...
            //return when user is not the one who claimed the ticket
            const claimedBy = ticket.get("opendiscord:claimed-by").value
            if (claimedBy && claimedBy !== user.id){
                instance.reply(await opendiscord.builders.messages.getSafe("opendiscord:error").build("button",{guild,channel,user,error:"Only the staff member who claimed this ticket can close it.",layout:"simple"}))
                return cancel()
            }
