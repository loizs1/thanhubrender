...
const panelEmbeds = () => {
    ...
    embeds.get("opendiscord:panel").workers.add(
        new api.ODWorker("opendiscord:panel",0,async (instance,params) => {
            ...
            instance.setFooter("Need help? Contact my dev @lois_nott/aeri • Powered by Thanhub Community")
...
const ticketEmbeds = () => {
    ...
    embeds.get("opendiscord:ticket-action-dm").workers.add(
        new api.ODWorker("opendiscord:ticket-action-dm",0,async (instance,params,source) => {
            ...
            //use original channel name from additionalData if available (for close DM), otherwise use current channel name
            const ticketName = (mode == "close" && typeof additionalData == "string") ? additionalData : (channel ? channel.name : "<unknown>")
            
            instance.setColor(generalConfig.data.mainColor)
            instance.setTimestamp(new Date())
            instance.addFields({name:lang.getTranslation("params.uppercase.ticket")+":",value:"```#"+ticketName+"```",inline:false})
...
const transcriptEmbeds = () => {
    ...
    embeds.get("opendiscord:transcript-text-ready").workers.add(
        new api.ODWorker("opendiscord:transcript-text-ready",0,async (instance,params,source) => {
            ...
            if (claimerId){
                try{
                    const claimer = await channel.client.users.fetch(claimerId)
                    instance.addFields({name:lang.getTranslation("stats.properties.claimedBy")+":",value:claimer.username+" ("+discord.userMention(claimer.id)+")"})
                }catch{}
            }
            if (closerId){
                try{
                    const closer = await channel.client.users.fetch(closerId)
                    instance.addFields({name:lang.getTranslation("stats.properties.closedBy")+":",value:closer.username+" ("+discord.userMention(closer.id)+")"})
                }catch{}
            }
            ...
            if (closeReason) instance.addFields({name:lang.getTranslation("params.uppercase.reason")+":",value:"```"+closeReason+"```",inline:false})
